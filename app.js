...
var addUser = function(data, callback) {
  speedOnlineDb.accounts.insert({
    username: data.username,
    password: data.password
  }, function(err) {
    speedOnlineDb.skillLvls.insert({
      username: data.username,
      skillLvl: 0
    }, function(err) {
      callback();
...
io.sockets.on('connection', function(socket) {
  ...
  socket.on("checkIfStaleMate", function(data) {
    ...
    if (count == 4) {
      ...
      for (var i in SOCKET_LIST) {
        SOCKET_LIST[i].emit('decksUpdated', {
          pileOne: sparePileOne,
          pileTwo: sparePileTwo,
          pOneHand: playerOneHand,
          pTwoHand: playerTwoHand,
          pOneDeck: playerOneDeck,
          pTwoDeck: playerTwoDeck,
          cardPileO: cardPileOne,
          cardPileT: cardPileTwo
        });
      }
      count = 0;
    } else {
  ...
  var findPlayerSkill = function(data, callback) {
    speedOnlineDb.skillLvls.find({
      username: data.username
    }, function(err, res) {
      if (res.length > 0) {
        console.log(res[0].skillLvl);
        socket.emit('getSkill', {
          skillLvl: res[0].skillLvl
        });
      }
    });
  }
  ...
  socket.on('gameStart', function(data) {
    if (numPlayers == 2) {
      flipSideDeckCard(cardPileOne, cardPileTwo, sparePileOne, sparePileTwo);
      for (var i in SOCKET_LIST) {
        SOCKET_LIST[i].emit('deckCreated', {
          pileOne: sparePileOne,
          pileTwo: sparePileTwo,
          pOneHand: playerOneHand,
          pTwoHand: playerTwoHand,
        ...
        SOCKET_LIST[i].emit('inputsActive', {
          inputs: true
        });
      }
      beginChecks = true;
